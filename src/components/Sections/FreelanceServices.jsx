        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {services.map((service, idx) => (
            <motion.div
              ...
              className="glass-panel-light p-8 rounded-2xl text-center border border-white/15 hover:border-accent/50 transition-all cursor-hover relative overflow-hidden group backdrop-blur-sm"
            >
              <div className="absolute inset-0 bg-gradient-to-br from-accent/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
              <div className="w-16 h-16 bg-accent/25 rounded-full flex items-center justify-center mx-auto mb-4 relative z-10">
                <i className={`fas ${service.icon || 'fa-code'} text-accent text-2xl`}></i>
              </div>
              <h3 className="text-xl font-bold text-white mb-2 relative z-10 drop-shadow-md">{service.title}</h3>
              <p className="text-gray-200 text-sm relative z-10 drop-shadow-md">{service.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
