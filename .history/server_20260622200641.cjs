...
app.post('/api/test-email', async (req, res) => {
  try {
    const mailOptions = {
      from: `"Portfolio Test" <${config.senderEmail}>`,
...
app.post('/api/send-email', async (req, res) => {
  ...
  try {
    ...
    console.log(`👤 From: ${sanitized.user_name} (${sanitized.user_email})`);
    ...
    if (error.code === 'EAUTH') {
      ...
      console.error('Connection failed. Check internet or SMTP settings.');
    }
