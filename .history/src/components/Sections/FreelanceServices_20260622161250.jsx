...
const FreelanceServices = ({ profile }) => {
  ...
  return (
    <section id="services" className="py-20 bg-[#111827] relative overflow-hidden">
      {/* Background Video - Using hero video as fallback, or add a freelanceVideo to profile.json */}
      {profile.heroVideo && (
        <video
          autoPlay
          loop
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {services.map((service, idx) => (
            <motion.div
              ...
              className="glass-panel p-8 rounded-2xl text-center border border-white/10 hover:border-accent/40 transition-all cursor-hover relative overflow-hidden group"
            >
